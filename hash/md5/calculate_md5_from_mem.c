...
size_t				calculate_md5_mem_padding(const char *str,
									size_t size, char **padded)
{
	size_t		len;
	size_t		padded_len;
...
t_hash_md5			calculate_md5_from_mem(void *ptr, size_t size)
{
	t_hash_md5			hash;
	char				*padded;
	char				*padded_cpy;
	...
	init_md5_hash(&hash);
	padded_len = calculate_md5_mem_padding(ptr, size, &padded);
	padded_cpy = padded;
	while (padded_len)
	{
